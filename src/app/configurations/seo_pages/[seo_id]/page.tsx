...
export default function Seo_id({ params }: { params: { seo_id: string } }) {
    ...
    const handleOgImageUpdate = (index: number, file: File) => {
        setOgImageIndexes(prev => [...prev, index]);
        setNewOgImages(prev => [...prev, file]);
        const updatedImages = [...existingOgImages];
        updatedImages[index] = URL.createObjectURL(file);
        setExistingOgImages(updatedImages);
    };

    const handleTwitterImageUpdate = (index: number, file: File) => {
        setTwitterImageIndexes(prev => [...prev, index]);
        setNewTwitterImages(prev => [...prev, file]);
        const updatedImages = [...existingTwitterImages];
        updatedImages[index] = URL.createObjectURL(file);
    ...
    // Função para upload de novas imagens OG
    const handleOgImageUpload = async (files: FileList) => {
        try {
            const apiClient = setupAPIClient();
            ...
            toast.success("Imagens OG adicionadas!");

        } catch (error) {
            toast.error("Erro ao adicionar imagens OG");
        }
    };

    // Função para upload de novas imagens OG
    const handleTwitterImageUpload = async (files: FileList) => {
        try {
            const apiClient = setupAPIClient();
            const formData = new FormData();
            formData.append('sEOSettings_id', params.seo_id);
